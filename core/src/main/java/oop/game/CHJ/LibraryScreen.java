...
//
//    public LibraryScreen(Main game, InventoryUI inventory) { // ← รับ inv เข้ามา
//        this.game = game;
...
//            game.setScreen(new FirstScreen(game, inventory));
//        }
//    }
//
...
public class LibraryScreen implements Screen {
    ...
    private boolean acOff = false;

    private Array<Rectangle> collisionRects;

    private MapLayer normalForeground;
    ...
    private void handleInput(float delta) {
        ...
        if (moving && walkFrames != null) {
            ...
            // ไม่เดิน → กลับเป็นท่ายืน
            if (walkFrames != null && walkFrames.length > 0) {
                currentTex = walkFrames[1];
            }
        }

        // สร้างกล่อง hitbox ของตัวละคร (ใหญ่ขึ้นเพื่อป้องกันลื่นผ่านกำแพง)
        float playerWidth = currentTex.getWidth() / 10f * 0.6f;
        ...
        // ตรวจสอบการชนพร้อมกัน (X+Y) ก่อนเคลื่อนที่
        ...
        for (Rectangle r : collisionRects) {
            if (nextRect.overlaps(r)) {
                willCollide = true;
                break;
            }
        }

        // ถ้าไม่ชน → เคลื่อนที่ทั้ง X และ Y
        if (!willCollide) {
            playerX = nextX;
            playerY = nextY;
        } else {
        ...
        if (bookVisible && Gdx.input.isKeyJustPressed(Input.Keys.Q)) {
            Rectangle playerRect = new Rectangle(playerX, playerY, 100, 150);
            Rectangle bookRect   = new Rectangle(1400, 1200, 40, 40);

            if (playerRect.overlaps(bookRect)) {
                boolean ok = inventory.collectItem("book");
                if (ok) {
                    game.questManager.onCollectItem("book");
                    bookVisible = false;
                }
